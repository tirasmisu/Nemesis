...
class BlacklistWordCommand extends BaseCommand {
    constructor() {
        super(
    ...
    async validateInput(interaction) {
        ...
        if (!/^[a-zA-Z0-9\s\-\'\.]+$/.test(word)) {
            console.log(`[BLACKLISTWORD] Invalid word format: "${word}"`);
    ...
    async executeCommand(interaction) {
        ...
        if (existingWord) {
            return { 
                error: true,
                message: `The word "${word}" is already in the blacklist.`
            };
        }
        ...
        const newBlacklistWord = new BlacklistWord({
            word: word.toLowerCase(),
            addedBy: interaction.user.id,
            punishmentId: punishmentId
        });
        
        await newBlacklistWord.save();
        console.log(`[BLACKLISTWORD] Added "${word}" to database blacklist`);
    ...
    async sendResponse(interaction, result) {
        ...
        if (result.error) {
            ...
            message = `The word "${result.word}" has been added to the blacklist.`;
        }
        ...
        try {
            if (interaction.deferred && !interaction.replied) {
                ...
                console.log(`[BLACKLISTWORD] Response: ${message}`);
            }
        } catch (error) {
            // If response fails, still log the success since database operation worked
            console.log(`[BLACKLISTWORD] Response failed but operation succeeded: ${message}`);
            console.log(`[BLACKLISTWORD] Error: ${error.message}`);
        }
    }
    ...
    async logAction(interaction, result) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: "⚠️ Word Blacklisted", value: `Word: \`${word}\``, inline: false },
...
module.exports = new BlacklistWordCommand();
